...
def stukje_troep():
    ...
    area.to_csv(r'F:\700 Georeferencing\Hendrik de Heer georeferencing/temp2.csv')
    
    area = pd.read_csv(r'F:\700 Georeferencing\Hendrik de Heer georeferencing/temp2.csv', header = None).drop(0, axis = 1)
    
    area = np.array(area)
    area = area[area < 5000]
    area = area[area > 200]
    
    area_hist = np.histogram(area, bins = 20)
    
    plt.hist(area, bins = [0,100,200,300,400,500,600,700,800,900,1000,1100,1200,1300,1400,1500]) 
    plt.title("histogram") 
    plt.show()
