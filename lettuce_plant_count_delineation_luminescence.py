...
def fit_kmeans_on_subset(img_path, kmeans_init):
    ...
    #get l band of subset and stack to make 2D input
    a = subset[:,2]
    b = subset[:,2]
    subset = None
    ...
    kmeans = KMeans(init = kmeans_init, n_jobs = -1, max_iter = 50, n_clusters = 5, verbose = 0, precompute_distances = False)
...
#input img_path
img_path = r'D:\700 Georeferencing\C49 georectification\Results/20190604_RGB_red_lettuce2.tif'
#output file directory
out_path = r'D:\700 Georeferencing\C49 georectification\Temp_results'
...
def cluster_objects(x_block_size, y_block_size, it, img_path, out_path, kmeans_init, iterative_fit, run_classification):    
    ...
    if iterative_fit == False:
        kmeans = fit_kmeans_on_subset(img_path, kmeans_dark_init)
    #time process
    tic = time.time()
